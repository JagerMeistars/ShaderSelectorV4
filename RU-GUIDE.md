...
**`SS_MARKER_TOLERANCE` (0)** — опциональный допуск сравнения каналов маркера; по умолчанию 0 (точное сравнение, поведение неизменно), при 1 каналы сигнатуры сопоставляются с `|diff|<=1` — включать при неработающих каналах на 26.3 (округление OIT ±1).
